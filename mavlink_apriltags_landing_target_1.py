...
def send_landing_target_packet(tag, w, h, tag_size):
    ...
    temp = struct.pack("<qfffffbb",
                       int(x),
                       y,#((tag.cx() / w) - 0.5) * h_fov,
                       z,#((tag.cy() / h) - 0.5) * v_fov,
                       0,#min(max(z_to_mm(tag.z_translation(), tag_size) / 1000, MAV_LANDING_TARGET_min_distance), MAV_LANDING_TARGET_max_distance),
                       0.0,
                       0.0,
                       0,
...
while(True):
    ...
    x = 10*math.sin(count/100)
    y = 15*math.cos(count/200)
    z = 20*math.cos(count/50)
    ...
    send_landing_target_packet(x, y, z,0) # valid_tag_ids[tags[0].id()])
    print("sending x and y: %.2f %.2f" % (x,y))
